...
public abstract class BibleSource 
{
	...
	abstract public int getNumberOfVerses(int bookIndex, int chapterIndex);
        ...
        /* Default verse/book number mapping functions */
	...
	public String getReferenceString(int bookIndex, int chapterIndex, int verseIndex)
	{
            if (TextStyle.reverseCharacters) {
                ...
                refStr.append(
                        goBible.localizeDigits(
                            "" + (getVerseNumberFromIndex(bookIndex, chapterIndex, verseIndex) + 1)
                            )
                        );
                refStr.append(":");
            ...
            return goBible.localizeDigits((chapterIndex + getStartChapter(bookIndex)) + "")
                    + ":"
                    + goBible.localizeDigits(
                        "" + (getVerseNumberFromIndex(bookIndex, chapterIndex, verseIndex) + 1)
                        );
