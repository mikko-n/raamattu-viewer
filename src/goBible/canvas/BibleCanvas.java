...
//	of the License, or (at your oPption) any later version.
//
...
import goBible.views.KeySettingsForm;
...
public class BibleCanvas extends SuperCanvas implements CommandListener, Runnable
{
	...
	private Command cancelCommand = new Command(GoBible.getString("UI-Cancel"), Command.CANCEL, 0);
        ...
        // Holds Rectangles to determine locations of linked words
        MapLink mapLinks = new MapLink();
	...
	// Colours
	private int textColour = 0x000000;
	private int backColour = 0xFFFFFF;
	private int christWordsColour = 0xD00000;
	private int highlightColour = 0xE0E0FF;
	...
	// Goto mode fields
        ...
        // selection[0] is 1-based
        // selection[1], selection[2] are 0-based
	...
	protected void pointerPressed(int x, int y)
	{
            ...
            System.out.println("[BibleCanvas.pointerPressed("+x+", "+y+")] called, fullscreen: "+goBible.fullScreen);
            if (mode == MODE_GOTO) {
...
//                if (mapLinks.contains(x, y) != null) {                                    
//                    goBible.showAlertMessage("KarttaSanaa klikattu! x"+x+" y"+y+" sana="+mapLinks.contains(x, y), AlertType.INFO);
//                    System.out.println("KarttaSanaa klikattu! x"+x+" y"+y+" sana="+mapLinks.contains(x, y));
////                    return;
//                }
                    ...
                    synchronized (getCurrentPassage()) {
                        ...
                        {                     
                           if (scrollingTimerTask == null) {                               
                               ...
                               pointerPressed(width-1, height/2);                               
...
//					enterGotoMode();
		...
		{                  
                    ...
                    if ((keyCode >= KEY_NUM0) && (keyCode <= KEY_NUM9))
                    {
                        ...
                        if (selectionIndex == 0) {
                            // because book numbers will be looked up, this needs sanitation
                            ...
                            if (impliedBook < 0) {
                                selection[0] = goBible.bibleSource.getBookNumberFromIndex(0);
                            }
                            else if (impliedBook >= numberOfBooks) {
                                selection[0] = goBible.bibleSource.getBookNumberFromIndex(numberOfBooks - 1);
                            }
                    ...
                    {                        
                        ...
                        switch (gameAction)
                        {
                            ...
                            /* in book ordering list, the numbers can be used to
                             * navigate directly to a book number. but up/down
                             * navigation should cycle through the actual ordering sequence.
                             *
                             * This means that a collection of NT + Psalms + Proverbs
                             * can be ordered Matt, Mark, ... Rev, Psa, Prov
                             * and cycled through in that manner, but each book
                             * retains its index as it would have in a complete bible.
                             */
                            case UP:
                            {
                                if (selectionIndex == SELECTION_BOOK) {
                                    ...
                                    impliedBook --;
                                    if (impliedBook < 0) {
                                        impliedBook = goBible.bibleSource.getNumberOfBooks() - 1;
                                    }
                                    ...
                                    impliedVerse --;
                                    if (impliedVerse < 0) {
                                        impliedVerse = goBible.bibleSource.getNumberOfVerses(impliedBook, selection[1]) - 1;
                                        System.err.println("Smaller than 0. Scrolling back to " + impliedVerse);
                                    }
                                    selection[2] = goBible.bibleSource.getVerseNumberFromIndex(impliedBook, selection[1], impliedVerse);
                                    System.err.println("Final number is " + selection[2]);
                                }
                                else {
                            ...
                            case DOWN:
                            {
                                ...
                                if (selectionIndex == 0) {
                                    int impliedBook = goBible.bibleSource.getBookIdFromNumber(selection[0]);
                                    impliedBook ++;
                                    if (impliedBook >= goBible.bibleSource.getNumberOfBooks()) {
                                        impliedBook = 0;
                                    }
                                    selection[selectionIndex] = goBible.bibleSource.getBookNumberFromIndex(impliedBook);
                                }
                                else if(selectionIndex == 2) {
                                    int impliedBook = goBible.bibleSource.getBookIdFromNumber(selection[0]);
                                    int impliedVerse = goBible.bibleSource.getVerseIndexFromNumber(impliedBook, selection[1], selection[2]);
                                    impliedVerse ++;
                                    if (impliedVerse >= goBible.bibleSource.getNumberOfVerses(impliedBook, selection[1])) {
                                        impliedVerse = 0;
                                    }
                                    selection[selectionIndex] = goBible.bibleSource.getVerseNumberFromIndex(impliedBook, selection[1], impliedVerse);
                                }
                                else {
                                    selection[selectionIndex]++;
                                }
        ...
        public void gotoFormRequest(int bookIndex, int chapterIndex, int verseIndex) {
	...
	private boolean validateSelection()
	{
		boolean correct = true;
                ...
                // Find the book/ verse number mappings:
                int impliedBook = goBible.bibleSource.getBookIdFromNumber(selection[SELECTION_BOOK]);
		
		// Validate the book index
		int numberOfBooks = goBible.bibleSource.getNumberOfBooks();
		
		if (impliedBook >= numberOfBooks)
		{
                    impliedBook = 0;
                    selection[SELECTION_BOOK] = goBible.bibleSource.getBookNumberFromIndex(impliedBook);
                    correct = false;
		}
		else if (impliedBook < 0)
		{
                    impliedBook = numberOfBooks - 1;
                    selection[SELECTION_BOOK] = goBible.bibleSource.getBookNumberFromIndex(impliedBook);
                    correct = false;
		}
		
		// Validate the chapter
		int numberOfChapters = goBible.bibleSource.getNumberOfChapters(impliedBook);
		...
		// Validate the verse
		int numberOfVerses = goBible.bibleSource.getNumberOfVerses(impliedBook, selection[SELECTION_CHAPTER]);
                int impliedVerse = goBible.bibleSource.getVerseIndexFromNumber(impliedBook, selection[SELECTION_CHAPTER], selection[SELECTION_VERSE]);

		if (impliedVerse >= numberOfVerses)
                {
                    impliedVerse = 0;
                    selection[SELECTION_VERSE] = goBible.bibleSource.getVerseNumberFromIndex(impliedBook, selection[SELECTION_CHAPTER], impliedVerse);
                    correct = false;
		}
		else if (impliedVerse < 0)
		{
                    impliedVerse = numberOfVerses - 1;
                    selection[SELECTION_VERSE] = goBible.bibleSource.getVerseNumberFromIndex(impliedBook, selection[SELECTION_CHAPTER], impliedVerse);
                    correct = false;
		}
...
//			}
            ...
            else if (mode == MODE_GOTO)
            {
                ...
                {
                    int book = goBible.bibleSource.getBookIdFromNumber(selection[SELECTION_BOOK]);
                    int chapter = selection[SELECTION_CHAPTER];
                    int verse = goBible.bibleSource.getVerseIndexFromNumber(book, chapter, selection[SELECTION_VERSE]);
                    userRequestPassage(
                            book,
                            chapter,
                            verse);
...
//                        goBible.showStackTrace(new Exception("Verse exceeds number of verses"), "paint()", currentPassage.toString());
	...
	public void paintGoto(Graphics g)
	{
            // Find the book/ verse number mappings:
            int impliedBook = goBible.bibleSource.getBookIdFromNumber(selection[SELECTION_BOOK]);
            int impliedChapter = selection[SELECTION_CHAPTER];
            int impliedVerse = goBible.bibleSource.getVerseIndexFromNumber(impliedBook, impliedChapter, selection[SELECTION_VERSE]);

            // Get the current chapter and verse
            String currentChapter = "" + (impliedChapter + goBible.bibleSource.getStartChapter(Math.max(0, impliedBook)));
            String currentVerse = "" + (selection[SELECTION_VERSE] + 1); // plus 1 because selection[*] is zero-based
            ...
            // Draw the book name
            g.drawString(bookNames[Math.max(0, impliedBook)], passageLeft, passageTop, Graphics.LEFT | Graphics.TOP);
	...
	public void drawButton(Graphics g, String label, Font font, int position)
	{
        ...
        private void paintReference(PaintContext context, PassageReference ctx) {
            ...
            boolean gradientToBlack = true;
            if (red < barHeight) gradientToBlack = false;
            g.fillRect(0, 0, width, barHeight/2);
            
            for (int i = barHeight/2; i <barHeight; i++ ) {                
                if (gradientToBlack) {
                    if (red>0) red = red-2;
                    if (green>0) green = green-2;
                    if (blue>0) blue = blue-2;                    
                } else {
                    if (red<255) red = red+2;
                    if (green<255) green = green+2;
                    if (blue<255) blue = blue+2;
                }                
//                goBible.Log("[BibleCanvas.paintReference] r"+red+" g"+green+" b"+blue+", to black? "+gradientToBlack);
                g.setColor(red, green, blue);                 
                g.drawLine(0, i, width, i);
            }            
            g.setColor(getHighlightColour()-getBackColour());
//            goBible.Log("[BibleCanvas.paintReference] last line: r"+g.getRedComponent()+" g"+g.getGreenComponent()+" b"+g.getBlueComponent());
            g.drawLine(0, barHeight, width, barHeight);


            String bookName = goBible.bibleSource.getBookName(ctx.getBookIndex());

            String referenceString = " " + goBible.bibleSource.getReferenceString(ctx.getBookIndex(), ctx.getChapterIndex(), ctx.getVerseIndex());
            ...
            if (TextStyle.reverseCharacters) { // reverse the order of appearance but not the digits
                ofStringBuffer.append(goBible.localizeDigits("" + goBible.bibleSource.getNumberOfVerses(ctx.getBookIndex(), ctx.getChapterIndex())));
...
//                        g.clipRect(0, 0, width - (BAR_PADDING + referenceStringWidth + ofStringWidth), height);
        ...
        private void normalizeReferences(PaintContext context, PassageReference ctx) {
            ...
            if (ctx.getLineOffset() < 0) {
                while (ctx.getLineOffset() < 0) {
                    ctx.setVerseIndex(ctx.getVerseIndex() - 1);
                    if (ctx.getVerseIndex() < 0) {
                        int oldOffset = ctx.getLineOffset();

                        ctx.previousChapter();
                        ctx.setLineOffset(oldOffset);
                        ctx.setVerseIndex(goBible.bibleSource.getNumberOfVerses(ctx.getBookIndex(), ctx.getChapterIndex()) - 1);
                    }
                    vLines = countLines(context, ctx);
                    System.out.println(Thread.currentThread().getName() + "[-] verse " + ctx.getVerseIndex() + " lines " + vLines);
                    ctx.setLineOffset(ctx.getLineOffset() + vLines);
                }
            }
            // Positive Line Offset. Scroll "Down"
                ...
                while (true) {
                    ...
                    if (ctx.getVerseIndex() >=
                            goBible.bibleSource.getNumberOfVerses(ctx.getBookIndex(), ctx.getChapterIndex())) {
                        System.out.println(Thread.currentThread().getName() + "Verse exceed limit: " + ctx);

                        int oldOffset = ctx.getLineOffset();
                        boolean newBook = (ctx.getChapterIndex() == goBible.bibleSource.getNumberOfChapters(ctx.getBookIndex()) - 1);
                        ctx.nextChapter();
...
//            }
        ...
        private int countAndPaintLines(boolean doPaint, PaintContext context, PassageReference ctx) {
            if (goBible.currentBookIndex != ctx.getBookIndex() || goBible.currentChapterIndex != ctx.getChapterIndex()) {
                System.out.println(Thread.currentThread().getName() + "UNOPTIMIZED Forced navigation");
                goBible.gotoPassage(ctx.getBookIndex(), ctx.getChapterIndex(), ctx.getVerseIndex());
            }

            int verseOffset = goBible.verseIndex[ctx.getVerseIndex() * 2];
            int verseEnd = goBible.verseIndex[ctx.getVerseIndex() * 2 + 1];
            char[] verse = goBible.verseData;

            // The line counter. No painting code will be called until currentLine >= 0
            int currentLine = - ctx.getLineOffset();

            StringBuffer verseBuffer;

            boolean needChapterHeading = ctx.getVerseIndex() == 0;
            String bookHeading = "\n\010" + goBible.bibleSource.getBookName(ctx.getBookIndex()) + "\010\n";

            String chapterNumber = goBible.localizeDigits(Integer.toString(ctx.getChapterIndex() + 1));
            if (TextStyle.reverseCharacters) {
                chapterNumber = new StringBuffer(chapterNumber).reverse().toString();
            }
            String chapterHeading = "\n\010" + GBCToolkit.replaceSubstring(
                    goBible.getString("UI-Chapter-Reference-%1"),
                    "%1",
                    chapterNumber
                    ) + "\010\n";
            boolean needBookHeading = needChapterHeading && ctx.getChapterIndex() == 0;
            boolean endOfBook = (ctx.getChapterIndex() == goBible.bibleSource.getNumberOfChapters(ctx.getBookIndex()) - 1) &&
                    (ctx.getVerseIndex() == goBible.bibleSource.getNumberOfVerses(ctx.getBookIndex(), ctx.getChapterIndex()) - 1);
            ...
            for (; charIndex < verseEnd; charIndex++) {
                ...
                if (charIsSpace || isNewline || endOfVerse) { // conditions to paint word
                    ...
                    while (segmWidth + context.x > context.wrapPoint) {
                        boolean useSoftHyphen = false;
                        int softHyphenPosition = 0;
                        
                        if (currentWord.softHyphens.size() > 0) {
                            for (int l=currentWord.softHyphens.size() - 1; l >= 0; l--) {
                                softHyphenPosition = ((Integer)currentWord.softHyphens.elementAt(l)).intValue();
                                segmWidth = currentWord.calculateWidth(context.textStyle, softHyphenPosition, true);
                                if (segmWidth + context.x <= context.wrapPoint) {
                                    useSoftHyphen = true;
                                    break;
                                }
                            }
                        ...
                        if(!useSoftHyphen && currentWord.canBreakBefore) {
                            ...
                            if (useSoftHyphen) { // we know where to break
                                finalPoint = softHyphenPosition + 1;
                            }
                            else { // we don't yet know where to break
                                /* do binary search to print long words */
                                while (true) {
                            ...
                            paintWord2(doPaint && currentLine >= 0, context, ctx, currentWord, wrapIndex, true);
                            System.out.println("Break word, printed: " + currentWord.wordBuffer.toString().substring(0, wrapIndex) + " " + useSoftHyphen + " " + softHyphenPosition);
                            ...
                            // delete used hyphenation points
                            int softHyphenCount = currentWord.softHyphens.size();
                            if (softHyphenCount > 0) {
                                Vector softHyphens2 = new Vector();
                                for (int i=0; i < softHyphenCount; i++) {
                                    int newPos = ((Integer)currentWord.softHyphens.elementAt(i)).intValue() - wrapIndex - 1;
                                    if (newPos > 0) {
                                        softHyphens2.addElement(
                                            new Integer(newPos));
                                    }
                                }
                                currentWord.softHyphens = softHyphens2;
                            }
                            // Break one line
                            if (doPaint && currentLine >= 0) {
                                context.y += TextStyle.fontHeight;
                    ...
                    paintWord2(doPaint && currentLine >= 0, context, ctx, currentWord, currentWord.wordBuffer.length(), false);
        ...
        private void paintWord2
            ...
            for (int i = 0; i <= stylesCount;  i++) {
                ...
                if (dropLineAfter && word.hasSoftHyphenAt(finalPosition)) {
                    s1 = s1 + "-";
                }
                ...
                if (doPaint) {
                    ...
                    // mikäli ollaan piirtämässä karttasanaa, otetaan talteen sanan vasen yläkulma karttarectanglea varten
                    // ja lasketaan rectanglen oikea alakulma sanan leveydestä + fontin korkeudesta                    
//                    if (drawMapLink(s1)) {    
...
//                            }
//                            ry = y;
//                            rw = context.textStyle.stringWidth(s1.trim());
//                            rh = context.textStyle.getHeight();
//                            
//                            // todo: tämän siirto TextStyle -luokan drawString -metodiin
//                            mapLinks.addWord(s1.trim(), new Rectangle(rx, ry, rw, rh));
//                            
//                            context.g.setColor(0xcccccc);
//                            context.g.drawRect(rx, ry, rw, rh);
//                            System.out.println("Word rectangle stored & drawn, word "+s1.trim()+", x" + rx + " y" + ry + " w" + rw + " h" + rh);
//                        } else {
//                            mapLinks.removeWord(s1.trim());
//                            System.out.println("Word "+s1.trim()+" rectangle out of sight, removed, not stored or drawn: x" + rx + " y" + ry + " w" + rw + " h" + rh);
//                        }
//                    }
                }
                x += width;
...
//        private String[] placesList = new String[] {"Jerusalem", "Jordan", "Juuda"};
        /**
         * Check word against predefined list
         * @param s
         * @return true if match
         */
//        private boolean drawMapLink(String s) {
////            System.out.println("DrawMaplink called, word "+s);
//            
//            for (int i = 0; i<placesList.length; i++) {
//                if (s.indexOf(placesList[i]) > -1) {
//                    return true;
//                }
//            }            
//            return false;
//        }
        ...
        private void paintCached(PaintContext context, PassageReference ctx) {
            ...
            if (caches[0] != null && caches[1] != null && lastDrawnPosition != null && (lastDrawnPosition.sameVerse(ctx))) {
                int lineDiff = (lastDrawnPosition.getLineOffset() - ctx.getLineOffset());
            ...
            Y: for (int i=0; i<2; i++) {
                ...
                for (int j = 0; j < caches[i].verseReferences.size(); j++) {
                    ...
                    if (r.sameVerse(ctx)) {
                        System.out.println(Thread.currentThread().getName() + "Requested and matched verse " + ctx.getVerseIndex());
                        System.out.println(Thread.currentThread().getName() + "  at Cache " + i + " Ref #" + j);
                        // actual line offset from the first line of first cache.
                        int lineOffsetDifference = ctx.getLineOffset() - r.getLineOffset() + r.lineIndex + (i * linesPerScreen);
                        ...
                        if (-linesPerScreen <= lineOffsetDifference && lineOffsetDifference < 0) {
                            ...
                            temp.setLineOffset(temp.getLineOffset() - linesPerScreen);
                        ...
                        X: for (int l = 0; l < 2; l++) {
                            for (int k = 0; k < caches[l].verseReferences.size(); k++) {
                                ...
                                if (currentRef.lineIndex + (l * linesPerScreen) > lineOffsetDifference) {
                                    ...
                                    lastDrawnPosition.setLineOffset(lineOffsetDifference - lastRef.lineIndex + lastRef.getLineOffset());
                                    break X;
                                }
                                lastRef = currentRef;
                            }
            ...
            while (taskList != null) {
                ...
                for (int i = 1; i < taskList.size(); i++) {
                    ...
                    synchronized (taskToUpdate) {
                        if (taskToUpdate.sameVerse(task)) {
                            int newLineOffset = taskToUpdate.getLineOffset() - task.getLineOffset() + simplerRef.getLineOffset();
                            ...
                            for (int j = 2; j < task.paintCache.verseReferences.size(); j++) {
                                ...
                                if (r.lineIndex >= newLineOffset) {
                                    taskToUpdate.setBookIndex(lastRef.getBookIndex());
                                    taskToUpdate.setChapterIndex(lastRef.getChapterIndex());
                                    taskToUpdate.setVerseIndex(lastRef.getVerseIndex());
                                    taskToUpdate.setLineOffset(newLineOffset - lastRef.lineIndex);
                                    break;
                                }
                                lastRef = r;
                            }
                ...
                synchronized (lastDrawnPosition) {
                    if (lastDrawnPosition.sameVerse(task)) {
                        lastDrawnPosition.setBookIndex(simplerRef.getBookIndex());
                        lastDrawnPosition.setChapterIndex(simplerRef.getChapterIndex());
                        lastDrawnPosition.setVerseIndex(simplerRef.getVerseIndex());
                        lastDrawnPosition.setLineOffset(lastDrawnPosition.getLineOffset() - (task.getLineOffset() - simplerRef.getLineOffset()));
                    }
                }
    ...
    /**
     * @return the christWordsColour
     */
    public int getChristWordsColour() {
        return christWordsColour;
    }
    ...
    /**
     * @param christWordsColour the christWordsColour to set
     */
    public void setChristWordsColour(int christWordsColour) {
        this.christWordsColour = christWordsColour;
    }

    /**
     * @param highlightColour the highlightColour to set
     */
    ...
    public PassageReference getCurrentPassage() {
        ...
        private class PWord {
            ...
            public Vector styles = new Vector(), softHyphens = new Vector();
            ...
            public void pushSoftHyphen() {
                if (wordBuffer.length() != 0) {
                    softHyphens.addElement(new Integer(wordBuffer.length()));
                }
            }
            private boolean hasSoftHyphenAt(int pos) {
                if (pos == 0) { // do not allow initial soft hyphens
                    return false;
                }
                for (int i=0; i<softHyphens.size(); i++) {
                    if (((Integer)softHyphens.elementAt(i)).intValue() == pos) {
                        return true;
                    }
                }
                return false;
            ...
            public int calculateWidth(TextStyle currentStyle, int length) {
                return calculateWidth(currentStyle, length, false);
            }
            public int calculateWidth(TextStyle currentStyle, int length, boolean countSoftHyphens) {
                ...
                for (int i = 0; i <= stylesCount;  i++) {
                    ...
                    String measuredString = this.wordBuffer.toString()
                            .substring(position, finalPosition);
                    if (countSoftHyphens && length == finalPosition && hasSoftHyphenAt(finalPosition)) {
                        measuredString += "-";
                    }
                    segmWidth += currentStyle.stringWidth(measuredString);
                    position = finalPosition;
                }
