                        ...
                        for (; (currentVerse < numberOfVerses) && (index >= 0) && (!stop) && (numberFound < MAX_RESULTS); currentVerse++)
                        {
                            ...
                            if (index >= 0)
                            {
                                                ...
                                                (!goBible.searchCanSpanMultipleVerses && containedInVerse))
                                {
                                    ...
                                    if (!GBCToolkit.isSpace(verseData[beforeIndex]))
                                    {
                                        int softLimit = beforeIndex - 7; // give a little more allowance

                                        while (
                                             beforeIndex > verseStart &&
                                             beforeIndex > softLimit &&
                                             !GBCToolkit.isSpace(verseData[beforeIndex - 1])
                                             )
                                        {
                                            if (GBCToolkit.isSurrogatePair(verseData[beforeIndex]) != 0) {
                                                softLimit--;
                                            }
                                            beforeIndex--;
                                        }
                                    ...
                                    if (!GBCToolkit.isSpace(verseData[afterIndex - 1]))
                                    {
                                        int softLimit = afterIndex + 7;

                                        while (
                                                afterIndex < verseEnd &&
                                                afterIndex < softLimit &&
                                                !GBCToolkit.isSpace(verseData[afterIndex]))
                                        {
                                            if (GBCToolkit.isSurrogatePair(verseData[afterIndex]) != 0) {
                                                softLimit++;
                                            }
                                            afterIndex++;
                                        }
		...
		for (int i = offset; i <= totalEnd && result == -1; i++)
		{
			...
			/*if ((currentChar >= 'A' && currentChar <= 'Z') || (currentChar >= 'А' && currentChar <= 'Я'))
			{
				currentChar += lowerDiff;
                        ...
                        else if (currentChar == 'І' || currentChar == 'Ї')
                        {
                            currentChar += lowerDiffUkr;
                        }*/
